...
export function useMulby(pluginId?: string) {
  return useMemo(() => ({
    ...
    scheduler: {
      ...
      cancel: (taskId: string) => window.mulby?.scheduler?.cancelTask(taskId),
      pause: (taskId: string) => window.mulby?.scheduler?.pauseTask(taskId),
      resume: (taskId: string) => window.mulby?.scheduler?.resumeTask(taskId),
      list: (filter?: { status?: string; type?: string }) => window.mulby?.scheduler?.listTasks(filter),
      get: (taskId: string) => window.mulby?.scheduler?.getTask(taskId),
    ...
    host: {
      invoke: (method: string, ...args: unknown[]) =>
        window.mulby?.host?.invoke(pluginId || 'scheduler-demo', method, ...args),
      call: (method: string, ...args: unknown[]) =>
        window.mulby?.host?.call?.(pluginId || 'scheduler-demo', method, ...args),
      status: () => window.mulby?.host?.status(pluginId || 'scheduler-demo'),
      restart: () => window.mulby?.host?.restart(pluginId || 'scheduler-demo'),
    },
